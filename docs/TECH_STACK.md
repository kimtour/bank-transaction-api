...
- **`app/static/`**: contains `index.html`, `styles.css`, `app.js`, `walkthrough.html` and `walkthrough.css`.
...
- **Pydantic**: validates request and response data before business logic runs.
...
- **Pytest**: automated Python testing framework used by the application suite.
...
- **Walkthrough tests**: confirm the build guide and its assets are served correctly.
...
- **`scripts/live_smoke.py`**: verifies the dashboard, health, Swagger, OpenAPI, registration, login, JWT authentication, account creation, transfer balances and transaction history on the public deployment.
...
- **`render.yaml`**: Render Infrastructure-as-Code configuration defining the hosted web service, Python version, build command, start command, health check and environment variables.
